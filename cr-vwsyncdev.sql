...
CREATE OR REPLACE VIEW vwsyncdev AS 
 SELECT m."КодСодержания",
   ...
   FROM devmod.modifications m,
    devmod.device d
  WHERE d.dev_id = m.dev_id AND d.version_num = m.version_num AND d.ie_xml_id IS NOT NULL AND d.ie_xml_id_dt IS NOT NULL 
  AND m."КодСодержания" IS NOT NULL AND m.version_num = 1
/*
UNION
