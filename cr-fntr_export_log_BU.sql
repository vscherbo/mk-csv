...
BEGIN
  ...
  THEN 
     IF (OLD.exp_status = 1 ) AND (NEW.exp_version_num > 1 )
     THEN
        -- TODO проверить, что изменяется только NEW.exp_version_num
        -- или вынести в отдельный триггер по полю exp_version_num
        NULL; -- разрешено завершённый экспорт переводить в архив
     ELSE
        RAISE E'Запрещено редактировать архив синхронизации.';
     END IF;
  ELSE
    ...
    ELSIF dev.ie_xml_id IS NULL AND NEW.exp_mod IN (1, 7) THEN
        NULL; -- разрешённая комбинация, идём дальше 
